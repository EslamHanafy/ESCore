...
//    let PHONE_REGEX = "^\\d{3}-\\d{3}-\\d{4}$"
    ...
    */
    public convenience init(message : String = "Enter a valid 10 digit phone number",showMessage: Bool = true) {
        self.init(regex: PhoneNumberRule.regex, message : message,showMessage:showMessage)
    }
